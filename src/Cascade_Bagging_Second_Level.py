    ...
    for i in range(model_num):
        if sam_mode == "folder":
            ...
            x_data,y_data, sam_x_real,sam_y_real = read_group_charac_bias_sample(data_root,bias,
                                                                                ...
                                                                                y_pro=y_pro)  
            for j in range(1):
#                 noise_x,noise_y = input_gaussian_noise(sam_x_real,sam_y_real,seed=j,SNR=1)  #仅31样本+噪声
                noise_x,noise_y = input_gaussian_noise(x_data,y_data,seed=j,SNR=1)    #231个相关样本全加噪声
